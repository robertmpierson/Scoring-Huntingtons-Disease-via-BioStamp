...
features_all=[];   
    ...
    % Row organization: [f1s1, f1s2... f1sn f2s1, f2s2, ...f2sn... fnsn],
...
reg_features = featuresHD(:,2/3*size(featuresHD,1)+1:end);
reg_features_test = featuresHD_test(:,2/3* size(featuresHD,1)+1:end);
...
% reg_features = featuresHD(:,2:8:end); % extract 39 features = (1 sensor location) x (13 features) x (3 activities) 
% reg_features_test = featuresHD_test(:,2:8:end); 
...
% reg_features = featuresHD
% reg_features_test = featuresHD_test
% reg_labels = sum(labels{i_trainHD,[11,12,14,18,21,22,23]},2);
% reg_labels_test = sum(labels{i_testHD,[11,12,14,18,21,22,23]},2);
% rng = [0,80];

disp('Selecting Features...')
[selected_fts, selected_test_fts, flabels]= selectFeats(reg_features,reg_features_test, ...
    reg_labels, reg_labels_test, featureTables.labels, taskList, type, dataDir, false);
...
for model_num= 1:length(modelList)
    ...
    [trn_ME, tst_ME, trntst_corrs] = getModelResults(chosenModel, ...
        ...
        type, selected_fts, selected_test_fts, reg_labels,...
